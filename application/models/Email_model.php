...
class Email_model extends CI_Model
{
    ...
    // DECLARATION: Send pdf invoice to customer.
    function send_invoice_to_customer($email_to, $cc = null, $attachment)
    {


        $email_msg = "Hi,<br>Here is your invoice. Please check the files attached.";
        $email_sub = 'Invoice';
        $email_to = $email_to;

        //var_dump($email_to);
        //die();
        $email_from = 'se@synergon.bg';
        $attachment = $attachment;
        $this->do_email($email_msg, $email_sub, $email_to, $cc, $email_from, $attachment);
    ...
    function do_email($msg = NULL, $sub = NULL, $to = NULL, $cc = NULL, $from = NULL, $attachment_url = NULL)
    {
        $config = array();
        $config['useragent'] = "CodeIgniter";
        $config['mailpath'] = "/usr/sbin/sendmail"; // or "/usr/sbin/sendmail"
        $config['protocol'] = "smtp";
        $config['smtp_host'] = $this->db->get_where('settings', array('type' => 'smtp_host'))->row()->description;
        $config['smtp_user'] = $this->db->get_where('settings', array('type' => 'smtp_user'))->row()->description;
        $config['smtp_pass'] = $this->db->get_where('settings', array('type' => 'smtp_pass'))->row()->description;
        $config['smtp_port'] = $this->db->get_where('settings', array('type' => 'smtp_port'))->row()->description;
